...
class EvaluationRun(Base, CreatedAtMixin):
    ...
    # Scores (0.0 – 1.0); nullable until evaluated
    truthfulness_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    role_fit_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    clarity_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    seniority_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
    integrated_score: Mapped[float | None] = mapped_column(Numeric(5, 4), nullable=True)
