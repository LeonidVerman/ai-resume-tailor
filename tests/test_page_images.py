...
_SAMPLE_40 = _PDF_DIR / "40-Resume-Sample-1-Software-Engineer.pdf"
...
# ---------------------------------------------------------------------------
# Label-column reconstruction tests
# ---------------------------------------------------------------------------

_DOCX_DIR = _HERE / "samples" / "resume" / "docx"
_SAMPLE_12_DOCX = _DOCX_DIR / "12-Nurse-template2.docx"
_SAMPLE_40_DOCX = _DOCX_DIR / "40-Resume-Sample-1-Software-Engineer.docx"

_W_NS = "http://schemas.openxmlformats.org/wordprocessingml/2006/main"


def _render_to_doc(ir, template_path: str):
    """Render *ir* to a temp DOCX and return the open python-docx Document."""
    import tempfile
    from tailor.compiler.docx_renderer import render_docx
    from docx import Document

    out = tempfile.mktemp(suffix=".docx")
    render_docx(ir, template_path, out)
    return Document(out), out


def _find_tbl_with_cols(doc_body, n_cols: int):
    """Return the first w:tbl in *doc_body* whose first row has *n_cols* cells."""
    for child in doc_body:
        if child.tag != f"{{{_W_NS}}}tbl":
            continue
        first_row = child.find(f"{{{_W_NS}}}tr")
        if first_row is None:
            continue
        cells = first_row.findall(f"{{{_W_NS}}}tc")
        if len(cells) == n_cols:
            return child
    return None
...
def _cell_text(tc) -> str:
    return "".join(t.text or "" for t in tc.iter(f"{{{_W_NS}}}t"))


@pytest.mark.skipif(
    not _SAMPLE_12.exists() or not _SAMPLE_12_DOCX.exists(),
    reason="sample 12 PDF/DOCX not in test fixtures",
)
class TestSample12LabelColumnReconstruction:
    """CONTACT INFO in sample 12 must be reconstructed as a 3-column label-row table.

    The original PDF layout:
        CONTACT INFO | 111 1st Avenue | 909.555.0100
                     | Redmond, WA    | kristi@example.com
                     | 65432          | www.interestingsite.com

    Before this fix the renderer emitted a standalone heading paragraph followed
    by a 2-column body table.  After the fix it emits a single N×3 table with
    the heading in the vertically-merged first column.
    """

    def _get_ir_and_doc(self):
        from tailor.compiler.pdf_parser import parse_pdf
        import os

        with open(_SAMPLE_12, "rb") as f:
            ir = parse_pdf(f.read())
        doc, out = _render_to_doc(ir, str(_SAMPLE_12_DOCX))
        self._tmp = out
        return ir, doc
    ...
    def test_contact_info_is_in_3col_table(self):
        _ir, doc = self._get_ir_and_doc()
        body = doc.element.body
        tbl = _find_tbl_with_cols(body, 3)
        assert tbl is not None, "Expected a 3-column table for CONTACT INFO label-column layout"
        first_row = tbl.find(f"{{{_W_NS}}}tr")
        cells = first_row.findall(f"{{{_W_NS}}}tc")
        heading_text = _cell_text(cells[0])
        assert "CONTACT" in heading_text.upper(), (
            f"First cell of 3-col table should contain heading 'CONTACT INFO', got {heading_text!r}"
        )

    def test_contact_info_heading_not_standalone_paragraph(self):
        """The 'CONTACT INFO' text must live inside a table cell, not a bare w:p."""
        _ir, doc = self._get_ir_and_doc()
        body = doc.element.body
        for child in body:
            if child.tag == f"{{{_W_NS}}}p":
                text = "".join(t.text or "" for t in child.iter(f"{{{_W_NS}}}t"))
                assert "CONTACT INFO" not in text.upper(), (
                    "CONTACT INFO must be inside a table cell, not a standalone paragraph"
                )

    def test_3col_table_has_address_and_phone_data(self):
        """Cells 1 and 2 of the 3-col table must contain address and phone data."""
        _ir, doc = self._get_ir_and_doc()
        body = doc.element.body
        tbl = _find_tbl_with_cols(body, 3)
        assert tbl is not None
        rows = tbl.findall(f"{{{_W_NS}}}tr")
        all_cell1 = " ".join(_cell_text(r.findall(f"{{{_W_NS}}}tc")[1]) for r in rows)
        all_cell2 = " ".join(_cell_text(r.findall(f"{{{_W_NS}}}tc")[2]) for r in rows)
        assert any(t in all_cell1 for t in ("Avenue", "1st", "Redmond", "65432")), (
            f"Col-1 of 3-col table should contain address data, got: {all_cell1!r}"
        )
        assert any(t in all_cell2 for t in ("555", "example", "www")), (
            f"Col-2 of 3-col table should contain phone/email data, got: {all_cell2!r}"
        )

    def test_h_rule_border_preserved_on_contact_info(self):
        """The CONTACT INFO heading inside the label-col table must still carry
        a w:pBdr/w:top border (h_rule anchor rendering must survive reconstruction)."""
        _ir, doc = self._get_ir_and_doc()
        body = doc.element.body
        tbl = _find_tbl_with_cols(body, 3)
        assert tbl is not None
        first_row = tbl.find(f"{{{_W_NS}}}tr")
        tc0 = first_row.findall(f"{{{_W_NS}}}tc")[0]
        # Find the heading paragraph in the first cell
        heading_p = tc0.find(f"{{{_W_NS}}}p")
        assert heading_p is not None
        pPr = heading_p.find(f"{{{_W_NS}}}pPr")
        pBdr = pPr.find(f"{{{_W_NS}}}pBdr") if pPr is not None else None
        top = pBdr.find(f"{{{_W_NS}}}top") if pBdr is not None else None
        assert top is not None, (
            "Heading inside label-col table should have w:pBdr/w:top (h_rule border)"
        )

    def test_vmerge_applied_to_heading_cell(self):
        """The heading cell in a multi-row label-col table must use w:vMerge."""
        _ir, doc = self._get_ir_and_doc()
        body = doc.element.body
        tbl = _find_tbl_with_cols(body, 3)
        assert tbl is not None
        rows = tbl.findall(f"{{{_W_NS}}}tr")
        if len(rows) < 2:
            pytest.skip("Only 1 data row — vMerge not needed")
        first_tc0 = rows[0].findall(f"{{{_W_NS}}}tc")[0]
        tcPr = first_tc0.find(f"{{{_W_NS}}}tcPr")
        vMerge = tcPr.find(f"{{{_W_NS}}}vMerge") if tcPr is not None else None
        assert vMerge is not None, "Row-0 heading cell must have w:vMerge"
        assert vMerge.get(f"{{{_W_NS}}}val") == "restart", (
            "Row-0 w:vMerge must have val='restart'"
        )


@pytest.mark.skipif(
    not _SAMPLE_40.exists() or not _SAMPLE_40_DOCX.exists(),
    reason="sample 40 PDF/DOCX not in test fixtures",
)
class TestSample40NoLabelColumnRegression:
    """Sample 40 'T E C H N I C A L  S K I L L S' must NOT be reconstructed as a
    label-column table.  The heading is a full-width title (col1_w=36pt < 50pt
    threshold); the two-col indented list must remain: heading para + 2-col body table.
    """
    ...
    def _get_doc(self):
        ...
        with open(_SAMPLE_40, "rb") as f:
            ir = parse_pdf(f.read())
        doc, out = _render_to_doc(ir, str(_SAMPLE_40_DOCX))
        self._tmp = out
        return doc
    ...
    def test_tech_skills_heading_is_standalone_paragraph(self):
        """The TECHNICAL SKILLS heading must appear as a standalone paragraph,
        not inside a 3-column table cell.  The heading text uses spaced letters
        ('T E C H N I C A L  S K I L L S'), so we search for 'T E C H' as a
        substring."""
        doc = self._get_doc()
        body = doc.element.body
        found_heading = False
        for child in body:
            if child.tag == f"{{{_W_NS}}}p":
                text = "".join(t.text or "" for t in child.iter(f"{{{_W_NS}}}t"))
                if "T E C H" in text or "TECHNICAL" in text.upper():
                    found_heading = True
                    break
        assert found_heading, (
            "Sample 40 TECHNICAL SKILLS heading must be a standalone paragraph "
            "(label-column reconstruction must NOT fire for this template)"
        )
